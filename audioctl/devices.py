...
def _collect_sysfx_snapshot(device_id):
    ...
    return snap
def _collect_registry_only_snapshot(device_id):
    """
    Registry-only snapshot for FX learn (no COM/PropertyStore).
    Safe for drivers/effects that crash on COM reads.
    """
    import datetime
    snap = {
        "time": datetime.datetime.now().isoformat(timespec="seconds"),
        "com": {"skipped": "fx-learn"},
        "propStore": {"skipped": "fx-learn"},
        "registry": [],
    }
    try:
        snap["registry"] = _dump_mmdevices_all_values(device_id)
