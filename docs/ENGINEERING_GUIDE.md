...
- Windows version info comes from `version.txt` (filevers/prodvers 1.5.1.3).
...
* **audioctl/cli.py**
    ...
    *   **New in v1.5.1.3:** Handles conditional JSON payload generation for `listen` commands to preserve backward compatibility while exposing routing info.
...
- `listen` payload contract (v1.5.1.3 update):
