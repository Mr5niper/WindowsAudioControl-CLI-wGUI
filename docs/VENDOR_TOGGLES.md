# Vendor Toggles Configuration Guide (v1.5.1.3)
...
This document reflects the behavior of `audioctl/vendor_db.py` in **v1.5.1.3**.
--------------------------------------------------------------------------------

## 1) What is vendor_toggles.ini? (v1.5.1.3)
...
### v1.5.1.3 nuance: shared rules + “registry truth” discovery
In v1.5.1.3, the INI is best thought of as a **rule library**, not just “a list of devices”.
...
## 2) Default location and permissions (v1.5.1.3)
...
In v1.5.1.3 the runtime policy is **vendor-only**:
...
- `subkey`
  ...
  - v1.5.1.3 reads/writes exactly this learned scope
- `devices`
  - A list of endpoint GUIDs associated with this entry (used for candidate filtering in some paths)
  - NOTE: v1.5.1.3 also uses registry signature truth; support/apply ultimately depends on signature match for the endpoint
...
Important runtime nuance in v1.5.1.3:
...
Config fields:
- `decider_index`
  - Stored/parsed, but the v1.5.1.3 readback logic primarily uses quorum + best-signal scoring.
- `quorum_threshold`
  - Default: 0.60
...
## 16) Best practices (v1.5.1.3 behavior-aware)
...
- If a driver uses `Properties` instead of `FxProperties`, ensure `subkey = Properties` is recorded; v1.5.1.3 will respect it.
