# Vendor Toggles Configuration Guide (v1.5.0.0)
...
This document reflects the behavior of `audioctl/vendor_db.py` in **v1.5.0.0**.
--------------------------------------------------------------------------------

## 1) What is vendor_toggles.ini? (v1.5.0.0)
...
### v1.5.0.0 nuance: shared rules + “registry truth” discovery
In v1.5.0.0, the INI is best thought of as a **rule library**, not just “a list of devices”.
...
## 2) Default location and permissions (v1.5.0.0)
...
In v1.5.0.0 the runtime policy is **vendor-only**:
...
- `subkey`
  ...
  - v1.5.0.0 reads/writes exactly this learned scope
- `devices`
  - A list of endpoint GUIDs associated with this entry (used for candidate filtering in some paths)
  - NOTE: v1.5.0.0 also uses registry signature truth; support/apply ultimately depends on signature match for the endpoint
...
Important runtime nuance in v1.5.0.0:
...
Config fields:
- `decider_index`
  - Stored/parsed, but the v1.5.0.0 readback logic primarily uses quorum + best-signal scoring.
- `quorum_threshold`
  - Default: 0.60
...
## 16) Best practices (v1.5.0.0 behavior-aware)
...
- If a driver uses `Properties` instead of `FxProperties`, ensure `subkey = Properties` is recorded; v1.5.0.0 will respect it.
