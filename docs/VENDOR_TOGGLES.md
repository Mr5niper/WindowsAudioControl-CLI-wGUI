# Vendor Toggles Configuration Guide (v1.5.2.0)
...
This document reflects the behavior of `audioctl/vendor_db.py` in **v1.5.2.0**.
--------------------------------------------------------------------------------
...
Starting in v1.5.1.1, the INI is best thought of as a **rule library**, not just “a list of devices”.
...
- `subkey`
  ...
  - reads/writes exactly this learned scope
- `devices`
...
Important runtime nuance in v1.5.2.0:
...
Config fields:
- `decider_index`
  - Stored/parsed, but the v1.5.2.0 readback logic primarily uses quorum + best-signal scoring.
- `quorum_threshold`
  - Default: 0.60
...
## 16) Best practices (v1.5.2.0 behavior-aware)
...
- If a driver uses `Properties` instead of `FxProperties`, ensure `subkey = Properties` is recorded; v1.5.2.0 will respect it.
